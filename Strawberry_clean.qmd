...
```{r}
library(dplyr)
survey_ca_chem <- survey_d_chem |> 
  filter(State == "CALIFORNIA")
survey_flo_chem <- survey_d_chem |> 
  filter(State == "FLORIDA")
survey_ca_chem <- survey_ca_chem %>%
  filter(!is.na(Value)) %>% 
  select(-State.ANSI,-Ag.District,-Ag.District.Code,-County.ANSI,-County,-watershed_code,-more,-other,-CV....)
survey_flo_chem <- survey_flo_chem %>%
  filter(!is.na(Value)) %>% 
  select(-State.ANSI,-Ag.District,-Ag.District.Code,-County.ANSI,-County,-watershed_code,-more,-other,-CV....)
...
```{r}
# Output as csv files
write.csv(straw_sur_calnif, "straw_sur_calnif.csv", row.names = FALSE)
write.csv(straw_sur_florida, "straw_sur_florida.csv", row.names = FALSE)
write.csv(straw_nono_calnif, "straw_nono_calnif.csv", row.names = FALSE)
write.csv(straw_nono_florida, "straw_nono_florida.csv", row.names = FALSE)
write.csv(survey_ca_chem, "survey_ca_chem.csv", row.names = FALSE)
write.csv(survey_flo_chem, "survey_flo_chem.csv", row.names = FALSE)
```
